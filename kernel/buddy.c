...
int bit_misset(char *array, int index) {
  index /= 2;
  char b = array[index/8];
  char m = (1 << (index % 8));
  return (b & m) == m;
...
void bit_mset(char *array, int index) {
  index /= 2;
  char b = array[index/8];
  char m = (1 << (index % 8));
  array[index/8] = (b ^ m);
...
bd_malloc(uint64 nbytes)
{
  ...
  bit_mset(bd_sizes[k].alloc, blk_index(k, p));
  for(; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k-1);   // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    bit_mset(bd_sizes[k-1].alloc, blk_index(k-1, p));
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_mset(bd_sizes[k].alloc, bi);  // free p at size k
    if (bit_misset(bd_sizes[k].alloc, bi)) {  // is buddy allocated?
...
bd_initfree_pair(int k, int bi, void *bd_left, void *bd_right) {
  ...
  if(bit_misset(bd_sizes[k].alloc, bi)) {
    // one of the pair is free
    free = BLK_SIZE(k);
    // if(bit_isset(bd_sizes[k].alloc, bi))
    if((addr(k, bi) >= bd_left) && ((addr(k, bi) + free) < bd_right))
